...
1. Все MPI-процессы независимо считывают полную матрицу входных данных. Каждый процесс имеет полную копию матрицы для вычислений
2. Каждый процесс вычисляет множество столбцов с которыми он будет работать и находит минимумы по столбцам для назначенных ему столбцов. В результате каждый процесс формирует вектор минимумов
...
5. Корневой процесс рассылает объединенный вектор минимумов всем процессам

Преимущества данной схемы по сравнению с подходом, где корневой процесс распределяет столбцы между процессами:
- минимальные коммуникационные задержки - отсутствует этап распределения данных

Данная схема оптимальна для задач с простыми вычислениями, когда расходы на передачу данных превышают выгоду от их распределения.
...
|-------------|-------|----------|---------|------------|
| seq         | 1     |  1.575   |   1.00  | N/A        |
| mpi         | 2     |  1.048   |   1.5   | 75%        |
| mpi         | 4     |  0.967   |   1.63  | 40.75%     |
