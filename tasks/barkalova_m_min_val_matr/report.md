...
2. Каждый процесс получает свой блок строк
3. Каждый процесс находит минимумы по столбцам в своем блоке строк. Формируется вектор локальных минимумов
4. MPI_Allreduce с операцией MPI_MIN объединяет локальные минимумы. Все процессы получают полный вектор глобальных минимумов
...
MPI_Scatterv распределяет данные матрицы от корневого процесса всем процессам блоками строк
Корневой процесс:
- делит матрицу на блоки строк
- отправляет каждому процессу его часть данных
Остальные процессы:
- получают только свои блоки строк для обработки

MPI_Allreduce объединяет локальные минимумы всех процессов. Каждый процесс отправляет свой вектор local_min и для каждого столбца выбирается минимальное значение из всех процессов. В итоге все процессы одновременно получают итоговый вектор global_min
